...
from safer_streets_core.spatial import get_force_boundary, get_census_boundaries
...
def main() -> None:
    ...
    with st.expander("Data"):
        dfview = st.dataframe(features.drop(columns="geometry"))
    ...
    def render_data() -> None:
        ...
        while True:
            if go:
                ...
                map.pydeck_chart(
                    pdk.Deck(map_style=st.context.theme.type, layers=[boundary_layer, layer], initial_view_state=view_state), height=720
                )
